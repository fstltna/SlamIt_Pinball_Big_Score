...
void main(void)  
{
  ...
  // vViewVec = normalize( view_position.xyz - Pos) ;  
  // vViewVec = normalize( -vViewDirection.xyz) ;    
   ...
   vec3 B=vReflect*m; 
   B=B*0.25+0.5;    
   gl_TexCoord[2]=vec4(B.x,-B.z,1.0,1.0); // reflection sphere mapping  
    	
   gl_TexCoord[0]=gl_MultiTexCoord0; // diffuse 
   gl_TexCoord[1]=gl_MultiTexCoord1; // global illumination  
   gl_FrontColor=gl_Color;    // used for texture combiners  
